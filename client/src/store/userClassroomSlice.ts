...
export const deleteClassUser = createAsyncThunk(
  "userClassroom/delete",
  async (id: string, thunkAPI): Promise<Classroom> => {
    const res = await axiosInstance.delete(`c/${id}`, {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(deleteClassUser.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
         state.classes.owner_class.splice(index, 1);
        }
      });
  },
});
